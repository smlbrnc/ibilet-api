...
export class AuthService {
    ...
    return this.handleRequest(
      async () => {
        ...
        if (data.user && dto.metadata) {
          const { full_name, phone, date_of_birth, gender, nationality, tc_kimlik_no } =
            dto.metadata;
    ...
    return this.handleRequest(
      async () => {
        const { error } = await this.supabase
          .getAnonClient()
          .auth.resetPasswordForEmail(dto.email, { redirectTo: dto.redirectTo });
  ...
  async updatePassword(token: string, dto: UpdatePasswordDto) {
    if (!token)
      this.throwError('UPDATE_PASSWORD_ERROR', 'Token bulunamadı', HttpStatus.UNAUTHORIZED);
    ...
    return this.handleRequest(
      async () => {
        ...
        const { error } = await this.supabase
          .getAdminClient()
          .auth.admin.updateUserById(sessionData.user.id, { password: dto.password });
      ...
      async () => {
        ...
        if (data.user) {
          ...
          if (metadata.full_name || metadata.name)
            profileUpdate.full_name = metadata.full_name || metadata.name;
          if (metadata.avatar_url || metadata.picture)
            profileUpdate.avatar_url = metadata.avatar_url || metadata.picture;
        ...
        this.logger.log({
          message: 'ID Token ile giriş başarılı',
          provider,
          userId: data.user?.id,
        });
        return { success: true, data: { user: data.user, session: data.session } };
